...
describe("reading empty runnable sh", () => {
		it("should return proper status code and text", async () => {
			const response = await fetch(`${address}/emptySh`, {
				method: "GET",
			});
			expect(response.status).toBe(599);
			expect(response.statusText).toBe("Failed resourceToPayload received for request url \"/emptySh\" empty httpString");
		})
})