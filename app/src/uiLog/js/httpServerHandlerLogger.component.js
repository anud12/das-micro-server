window.customElements.define('http-server-handler-logger', class extends HTMLElement {
    indexRoot = document.createElement('http-server-handler-logger-index');
    ...
    connectedCallback() {
        ...
        ">
            <div>
                <section-start>Http server handler</section-start>
                <request-component data="request"></request-component>
                <div data="requesthandlerlogger"></div>
    ...
    update(props) {
        ...
        const requestHandlerLogger = props.requestHandlerLogger ?? {};
        const nodeTag = requestHandlerLogger?._type?.replace(/[A-Z]/g, letter => `-${letter.toLowerCase()}`);
        if (!nodeTag) {
            return;
        }
        ...
        if (!node) {
            ...
            this.querySelector('*[data=\'requesthandlerlogger\']').appendChild(node)
        }
        node.update(requestHandlerLogger);
        this.indexRoot.update(props);
    }

})
window.customElements.define('http-server-handler-logger-index', class extends HTMLElement {
    shadow;
    ...
    connectedCallback() {
        ...
        shadow.innerHTML = `
        <div active>
            <span counter></span>:<span method></span>:<span url></span>
        </div>
        <style>
            *[active] {
                user-select: none;
